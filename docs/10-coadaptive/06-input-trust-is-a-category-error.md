...
The problem is the category rather than the quality of anyone's parser. When the same channel carries data and instructions, no defensive parser can reliably tell them apart, because the distinction does not exist in the channel. You can sand the surface forever and never reach the bottom.
...
LLM inference treats every token in the context window as material it may act on. There is no input/instruction boundary inside the model. Wrapping user content in tags, prepending system prompts, fine-tuning for "ignore the user's directives": these are surface conventions rather than architectural separations. The attacker controls part of the context window; the attacker controls part of the program.
...
Read together, they point at one conclusion: prompt injection is a property of the [substrate](01-three-layer-model.md#substrate) rather than a bug you patch. The defenders who get this earliest stop investing in prompt-side hardening and start investing in capability containment.
